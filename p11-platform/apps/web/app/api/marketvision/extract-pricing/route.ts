/**
 * MarketVision 360 - AI Floor Plan & Pricing Extraction API
 * Extracts structured pricing/floor plan data from pasted competitor website content
 */
...
Analyze the following pasted content from a competitor's apartment website and extract all floor plan/unit type information.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If action is 'save' and competitorId provided, save to database
    if (action === 'save' && competitorId) {
      const savedUnits = await saveExtractedUnits(supabase, competitorId, validatedUnits)
    ...
    console.error('MarketVision Extract Pricing Error:', error)
    return NextResponse.json({ 
...
async function saveExtractedUnits(
  ...
  for (const unit of units) {
    // Check if unit type already exists for this competitor
      ...
      if (!error && updatedUnit) {
        ...
        if (priceChanged || existingUnit.available_count !== unit.availableCount) {
          await supabase.from('competitor_price_history').insert({
            competitor_unit_id: existingUnit.id,
            rent_min: unit.rentMin,
            rent_max: unit.rentMax,
            available_count: unit.availableCount,
